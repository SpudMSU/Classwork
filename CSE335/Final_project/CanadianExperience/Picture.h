...
public:
    ...
    */
	...
	///function to add tree to vector
	///\param tree
	void AddTree(std::shared_ptr<CTreeAdapter> tree) { mTrees.push_back(tree); }

	///function to add the basket
	///\param basket
	void AddBasket(std::shared_ptr<CBasketAdapter> basket) { mBasket = basket; }

	///function to get the trees
	///\return the trees
	std::vector<std::shared_ptr<CTreeAdapter>> GetTrees() { return mTrees; }

	///getter for the basket
	///\return the basket
	std::shared_ptr<CBasketAdapter> GetBasket() { return mBasket; }
