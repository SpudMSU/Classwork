...
BEGIN_MESSAGE_MAP(CViewEdit, CScrollView)
    ...
    ON_WM_LBUTTONDOWN()
    ON_WM_MOUSEMOVE()
	ON_COMMAND(ID_EDIT_HARVEST, &CViewEdit::OnEditHarvest)
	ON_COMMAND(ID_EDIT_EMPTYBASKET, &CViewEdit::OnEditEmptybasket)
	ON_COMMAND(ID_EDIT_SEEDTREE, &CViewEdit::OnEditSeedtree)
END_MESSAGE_MAP()
/// @endcond
...
void CViewEdit::OnEditHarvest()
{
	auto trees = GetPicture()->GetTrees();
	auto basket = GetPicture()->GetBasket();

	for (auto i : trees)
	{
		basket->AddToBasket(i->Harvest());
	}
	UpdateObserver();
}


void CViewEdit::OnEditEmptybasket()
{
	auto basket = GetPicture()->GetBasket();
	basket->EmptyBasket();
	UpdateObserver();
