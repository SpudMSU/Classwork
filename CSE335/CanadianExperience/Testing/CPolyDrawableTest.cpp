...
namespace Testing
{
	...
	public:
		...
		{
			CPolyDrawable drawable(L"ted");
			Assert::IsTrue(drawable.GetColor().GetValue() == Gdiplus::Color(0,0,0).GetValue());
			Gdiplus::Color newColor = Gdiplus::Color(100, 100, 100);
			drawable.SetColor(newColor);
			Assert::IsTrue(drawable.GetColor().GetValue() == newColor.GetValue());
		}
