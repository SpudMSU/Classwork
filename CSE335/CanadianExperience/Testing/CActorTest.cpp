...
namespace Testing
{
	...
	public:
		...
		TEST_METHOD(TestActorConstructor)
		{
			std::wstring myName = L"fred";
			CActor actor(myName);
			Assert::AreEqual(actor.GetName(), myName);
		}
		TEST_METHOD(TestEnabled)
		{
			CActor actor(L"ted");
		...
		{
			CActor actor(L"ted");
			Assert::IsTrue(actor.GetPosition().X == 0);
			Assert::IsTrue(actor.GetPosition().Y == 0);
			actor.SetPosition(Gdiplus::Point(10,15));
			Assert::IsTrue(actor.GetPosition().X == 10);
			Assert::IsTrue(actor.GetPosition().Y == 15);
		}
		...
		TEST_METHOD(TestCActorAnimation)
		{
			...
			actor->SetPosition(Gdiplus::Point(1234, 9833));
			...
			actor->SetPosition(Gdiplus::Point(101, 655));
			actor->SetKeyframe();
			...
			actor->SetPosition(Gdiplus::Point(202, 1000));
			actor->SetKeyframe();
